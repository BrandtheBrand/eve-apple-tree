...
- 🗺️ **A forest that arranges itself, until you'd rather it didn't** — trees you've never moved take cells on a checker grid that widens ring by ring as the forest grows, rather than stretching into a row where every extra tree shrinks all the icons. Trees joined by a bridge are placed on the same patch of ground. Drag a tree **anywhere** and it stays there; drop it **on** another tree and the two swap.
...
- 🔭 **Zoom scale** — four stops down the right edge: 🌲 whole forest · 🌳🌳🌳 a few trees · 🌳 one tree · 🍃 close on a leaf. It shows where you are and flies you there when clicked; a stop your vault can't express is dimmed rather than dead. Detail follows the stops — icons and tree names far out, titles at one tree, descriptions up close.
- 🌫️ **Two field modes:** flat ground wedges, or translucent vertical **columns** per field.
- 🌰 **Seeds and the seed pool** — a topic you haven't opened yet is a `seed`: it waits as a green, lit dot in a wide matte pool on the ground instead of claiming a place on the tree. Seeds with a `field` pool at that tree's foot; the rest gather in one shared pool at the middle of the forest. Point at a seed and it names itself; move away and the name is gone.
...
- 👁 **Stakeholder views, on the dot** — a note can carry a `## Stakeholder views` section (different "chairs" and their takes). Its card shows both halves: the chairs that have spoken on that idea, and **the ones that have not** — a chair that never speaks is a perspective you haven't taken. The cast is the owning tree's, so one tree's chairs never show up on another's.
...
- 🙈 **Folders to keep off the tree** — a setting for folders that stay in your vault, searchable and copyable, but never become dots: template packs, archives, reference material you keep nearby but aren't thinking about.
